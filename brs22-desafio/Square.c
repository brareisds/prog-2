...
square* square_create(unsigned char side, unsigned char face, unsigned short x, unsigned short y, unsigned short max_x, unsigned short max_y){			//Implementação da função "square_create"
	...
	new_square->lifeCount = MAX_LIVES;																													//Atribui vidas ao jogador (!)
	
	return new_square;																																	//Retorna o novo quadrado
}

void square_move(square *element, char steps, unsigned char trajectory, unsigned short max_x, unsigned short max_y, int NewUpperYScreen){				//Implementação da função "square_move" com o novo parametro (!)
	...
	else if (trajectory == 2){ if ((element->y - steps*SQUARE_STEP) - element->side/2 >= NewUpperYScreen) element->y = element->y - steps*SQUARE_STEP;}	//Verifica se a movimentação para cima é desejada e possível (com um novo limite superior); se sim, efetiva a mesma (!)
