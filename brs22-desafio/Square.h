...
#define MAX_LIVES 3																													//Tamanho, em pixels, de um passo do quadrado
...
typedef struct {																																	//Definição da estrutura de um quadrado
	...
	unsigned short lifeCount;																														// Elemento que armazena a quantidade de vidas dos jogadores (!)
...
void square_move(square *element, char steps, unsigned char trajectory, unsigned short max_x, unsigned short max_y, int NewUpperYScreen);								//Protótipo da função de movimentação de um quadrado
